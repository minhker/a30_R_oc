...
flash_dtb() {
  ...
  if [ "$dtb" ]; then
    dtbblock=/dev/block/platform/13500000.dwmmc0/by-name/dtb$slot;
    if [ ! -e "$dtbblock" ]; then
      abort "dtb partition could not be found. Aborting...";
    fi;
    if [ -f "$bin/flash_erase" -a -f "$bin/nandwrite" ]; then
      $bin/flash_erase $dtbblock 0 0;
      $bin/nandwrite -p $dtbblock $dtb;
    elif [ "$customdd" ]; then
      dd if=/dev/zero of=$dtbblock 2>/dev/null;
      dd if=$dtb of=$dtbblock;
    else
      cat $dtb /dev/zero > $dtbblock 2>/dev/null || true;
    fi;
    if [ $? != 0 ]; then
      abort "Flashing dtb failed. Aborting...";
    fi;
  fi;
}
### write_boot (repack ramdisk then build, sign and write image and dtbo and dtb)
write_boot() {
  repack_ramdisk;
  flash_boot;
  flash_dtbo;
